...
class BinarySearchTree {
 ...
 private:
  ...
  void SearchHalfFullNodes(Node* runner) {
    if (runner) {
      SearchHalfFullNodes(runner->left_child);
      if ((!runner->left_child && runner->right_child) || (runner->left_child && !runner->right_child)) {
        std::cout << runner->value << std::endl;
      }
      SearchHalfFullNodes(runner->right_child);
    }
  }
